...
4. **SubValueChecker 클래스:**
    ...
    - **필요성:** 중첩된 데이터에서 특정 값(Value)의 유무를 검증한다.
5. **ValueChecker 클래스:**
...
3. **TypeSplitter 클래스:**
    - **설명:** Sensor Type마다 JSON 객체를 Key-Value 쌍으로 분리하는 클래스이다.
...
- JSONObject를 생성하여 메세지로 변환 후 포트안에 넣어준다.
- SensorType Splitter를 사용하여 Type에 따라 Message를 생성한다.
- Null Value Filter에서는 NullCheck를 사용하여 해당 Sensor값을 사용한다.
